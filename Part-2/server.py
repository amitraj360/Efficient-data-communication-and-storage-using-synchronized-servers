...
#opening server.txt in read mode
...
s.listen()# server waiting for connection

server_sync_delay = 10  #sync delay between the server
    ...
    def run(self):
        while True:
            ...
            for port_no in LIST_SERVERS_PORT:
                try:
                    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    #.socket-> It specify the adress family
                ...
                for file_detail in data_files:
                    ...
                    print("Alive")  #cheakpoint 2


                    #for updatation on own server ,if both server has same file

                    if os.path.exists(SERVER_PATH + file_name):

                        file_metadata = open(SERVER_PATH + ".metadata-" + file_name, "r") #'r' error if file will not exist
                        my_file_modified_time = float(file_metadata.readline()[:-1])
                        file_metadata.close()

                        #updating WRT to another server file
                    ...
                    file_output = open(SERVER_PATH + file_name, "w")  #if file not exist it will create file with this name
...
class ThreadTask(threading.Thread):
    ...
    def run(self):
        ...
        if command == b'1':
            ...
            file_name = self.conn.recv(256).decode('ascii')
            LOCKS_FILES[file_name] = True #newly updated file
        self.conn.close()
    ...
    def sync_server(self):
        ...
        for file_name in os.listdir(SERVER_PATH):
            if not file_name.startswith(".") and LOCKS_FILES[file_name]:
                ...
                #tell will tell us how much byte our read write head moved
                #IN begning read-write head always pointing to the starting of the file
                file_input.close()
                file_metadata.close()
        self.conn.send(result.encode('ascii'))
